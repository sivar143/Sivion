...
export interface InventoryTransaction {id:number;productId:number;warehouseId:number;transactionType:string;quantity:number;referenceType?:string;referenceId?:number;correlationId:string;createdAt:string}
@Injectable({providedIn:'root'}) export class InventoryApi {
 ...
 async createWarehouse(code:string,name:string){return firstValueFrom(this.http.post<Warehouse>(`${this.base}/warehouses`,{code,name},await this.options()));}
 ...
 async transactions(){return firstValueFrom(this.http.get<InventoryTransaction[]>(`${this.base}/transactions`,await this.options()));}
}
