...
@Injectable({providedIn:'root'}) export class InventoryApi {
 ...
 async stockIn(productId:number,quantity:number,reference:string){return firstValueFrom(this.http.post<void>(`${this.base}/stock-in`,{productId,warehouseId:1,quantity,reference},await this.options()));}
 async adjustment(productId:number,quantity:number,reference:string){return firstValueFrom(this.http.post<void>(`${this.base}/adjustments`,{productId,warehouseId:1,quantity,reference},await this.options()));}
