...
public class SalesOrderService {
    ...
    private static final Set<String> STATUSES=Set.of("DRAFT","CONFIRMED","RESERVATION_FAILED","RESERVED","PARTIALLY_DISPATCHED","DISPATCHED","CANCELLED");
    private final SalesOrderRepository repository; private final RabbitTemplate rabbit;
    ...
    public SalesOrder create(OrderRequest request){
        ...
        order.setDeliveryAddress(request.deliveryAddress()); order.setNotes(request.notes()); order.setOrderNumber(nextNumber()); BigDecimal total=BigDecimal.ZERO;
        for(ItemRequest r:request.items()){
            ...
            item.setLineTotal(r.quantity().multiply(r.unitPrice())); order.addItem(item); total=total.add(item.getLineTotal());
        }
    ...
    public SalesOrder updateStatus(Long id,String status){
        ...
        if("RESERVED".equals(next)||"RESERVATION_FAILED".equals(next)||"PARTIALLY_DISPATCHED".equals(next)||"DISPATCHED".equals(next))throw new IllegalStateException("This order status is managed by domain events");
        if("DRAFT".equals(current)&&!("CONFIRMED".equals(next)||"CANCELLED".equals(next)))throw new IllegalStateException("Invalid transition from DRAFT to "+next);
        if("CONFIRMED".equals(current)&&!"CANCELLED".equals(next))throw new IllegalStateException("Confirmed orders are awaiting inventory reservation");
        if("RESERVATION_FAILED".equals(current)&&!("CONFIRMED".equals(next)||"CANCELLED".equals(next)))throw new IllegalStateException("Invalid transition from RESERVATION_FAILED to "+next);
        if("RESERVED".equals(current)&&!"CANCELLED".equals(next))throw new IllegalStateException("Reserved orders cannot be manually changed");
        order.setStatus(next);
        if("CONFIRMED".equals(next)&&!"CONFIRMED".equals(current)){
            rabbit.convertAndSend(RabbitTopology.EXCHANGE,"sales.order.confirmed",Map.of("salesOrderId",order.getId(),"items",order.getItems().stream().map(i->Map.of("productId",i.getMaterialId(),"quantity",i.getQuantity())).toList()));
        }
        return order;
    }

    @Transactional public void markReserved(Long id){repository.findByIdAndTenantId(id,TENANT).ifPresent(o->{if("CONFIRMED".equals(o.getStatus()))o.setStatus("RESERVED");});}
    @Transactional public void markReservationFailed(Long id){repository.findByIdAndTenantId(id,TENANT).ifPresent(o->{if("CONFIRMED".equals(o.getStatus()))o.setStatus("RESERVATION_FAILED");});}

    @Transactional
    public void markDispatched(Long id){
        repository.findByIdAndTenantId(id,TENANT).ifPresent(o->{
            if("CANCELLED".equals(o.getStatus())||"DISPATCHED".equals(o.getStatus())) return;
            if(!"RESERVED".equals(o.getStatus())&&!"PARTIALLY_DISPATCHED".equals(o.getStatus())) return;
            o.setStatus("DISPATCHED");
            rabbit.convertAndSend(RabbitTopology.EXCHANGE,"sales.order.dispatched",Map.of("salesOrderId",o.getId(),"customerId",o.getCustomerId(),"amount",o.getTotalAmount()));
        });
    }
